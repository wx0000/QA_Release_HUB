...
export function TestCasesTable() {
  ...
  const handleResultChange = (nr: number, value: string) => {
    setCurrentResults((prev) => ({ ...prev, [nr]: value }))
  }
              ...
              {changes.map((c, i) => (
                ...
                >
                  ...
                  <td className="px-2 py-1.5">
                    <textarea
                      value={currentResults[c.nr] ?? ''}
                      onChange={(e) => handleResultChange(c.nr, e.target.value)}
                      placeholder="Describe test result…"
                      rows={2}
                      className="w-full bg-transparent text-text-primary text-xs px-1 py-0.5 rounded resize-none focus:outline-none focus:bg-bg-primary/40 focus:ring-1 focus:ring-accent/40 transition-all placeholder-text-muted"
                    />
                  </td>
                  <td className="px-3 py-2">
